...
class ReadCurrentFlow extends Command
{
    ...
    //        Log::info('Binary received (in hex):   ' . unpack('H*', $binaryData)[1]);
        ...
        if(Cache::get('test:mode')) {
            
            $testRodentId = Cache::get('test:rodent_id');

            if( ! $testRodentId || $testRodentId > 12) {
                $testRodentId = 1;
                Cache::put('test:rodent_id', $testRodentId);
            }

            $this->currentFlow['next_rodent_id'] = $testRodentId === 12 ? 1 : $testRodentId + 1;
            $this->currentFlow['rodent_id']      = $testRodentId;
            $this->currentFlow['current_flow']   = round(rand(100,6000),1);
            $this->currentFlow['date_time'] = (string)Carbon::now();
            $this->currentFlow['status'] = '11111111';


                Cache::put('test:rodent_id', $testRodentId + 1);
        }

...
class FlowRepository
{
    ...
    public function getPanelGraphData()
    {
        ...
        if(Cache::has('test:mode')) {
            ...
            $flow->whereRaw("CAST(DatumVreme as datetime) >= '" . $time . "'")
            ->whereRaw("CAST(DatumVreme as datetime) < '" . $time->addDay() . "'");
        } else {
            $flow->whereRaw("CAST(DatumVreme as datetime) >= '" . $this->getStartDateTime() . "'");
        }
                
        return $flow->groupBy('StationId', 'Smena')
            ->get()
            ->groupBy('station_id');
    }
